...
- La CVA multi-période (`computeMultiPeriodCva`) existe, par défaut avec une courbe de crédit simplifiée (`CreditCurve`, taux de hasard constant extrapolé depuis la seule PD à 1 an). Une courbe multi-points (`PiecewiseCreditCurve`, taux de hasard constant par segment entre points fournis) existe désormais en alternative, passable via `CvaAssumptions(cumulativeProbabilityOfDefault = ...)` — toujours construite à la main, pas une vraie courbe de marché ou d'agence.
- L'exposition simulée dans le temps existe en deux versions : `simulateExposureProfile` (brute) et `simulateExposureProfileWithCollateral` (avec remargining symétrique seuil/MTA). La fréquence de remargining (`CollateralAgreement.marginingStepsPerYear`) peut être plus grossière que le pas de simulation, une décote `AssetClass` et/ou une décote FX (`CollateralAgreement.collateralCurrency` + `fxHaircutTable`, combinées de façon additive comme dans `computeExposure`) peuvent être appliquées au collatéral posté, et le collatéral détenu peut désormais aussi évoluer comme un second facteur de risque brownien corrélé à l'exposition (`CollateralAgreement.collateralRiskFactor`), plutôt que de rester un simple solde déduit uniquement des appels de marge. Reste sans dérive et sans changement de composition du netting set dans le temps — pas un vrai moteur IMM.
