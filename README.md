...
- L'exposition simulée dans le temps existe en deux versions : `simulateExposureProfile` (brute) et `simulateExposureProfileWithCollateral` (avec remargining symétrique seuil/MTA, collatéral détenu comme solde stateful déduit du seul facteur exposition, pas un second facteur stochastique corrélé). La fréquence de remargining (`CollateralAgreement.marginingStepsPerYear`) peut désormais être plus grossière que le pas de simulation, et une décote `AssetClass` peut désormais être appliquée au collatéral posté (`CollateralAgreement.collateralAssetClass`), réduisant la couverture effective de chaque appel de marge. Reste un seul facteur de risque brownien sans dérive, sans changement de composition du netting set dans le temps, et sans décote FX sur le collatéral posté — pas un vrai moteur IMM.
