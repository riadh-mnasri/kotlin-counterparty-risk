...
- L'exposition simulée dans le temps existe en deux versions : `simulateExposureProfile` (brute) et `simulateExposureProfileWithCollateral` (avec remargining symétrique seuil/MTA, collatéral détenu comme solde stateful déduit du seul facteur exposition, pas un second facteur stochastique corrélé). Reste un seul facteur de risque brownien sans dérive, sans changement de composition du netting set dans le temps, sans décote/FX sur le collatéral posté, et sans fréquence de remargining distincte du pas de simulation — pas un vrai moteur IMM.
