...
- L'exposition simulée dans le temps existe en deux versions : `simulateExposureProfile` (brute) et `simulateExposureProfileWithCollateral` (avec remargining symétrique seuil/MTA). La fréquence de remargining (`CollateralAgreement.marginingStepsPerYear`) peut être plus grossière que le pas de simulation, une décote `AssetClass` peut être appliquée au collatéral posté (`CollateralAgreement.collateralAssetClass`), et le collatéral détenu peut désormais aussi évoluer comme un second facteur de risque brownien corrélé à l'exposition (`CollateralAgreement.collateralRiskFactor`), plutôt que de rester un simple solde déduit uniquement des appels de marge. Reste sans dérive, sans changement de composition du netting set dans le temps, et sans décote FX sur le collatéral posté — pas un vrai moteur IMM.
