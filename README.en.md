...
- Simulated exposure over time comes in two flavors: `simulateExposureProfile` (raw) and `simulateExposureProfileWithCollateral` (with symmetric threshold/MTA re-margining). The margining frequency (`CollateralAgreement.marginingStepsPerYear`) can be coarser than the simulation's own time step, an `AssetClass` haircut and/or an FX haircut (`CollateralAgreement.collateralCurrency` + `fxHaircutTable`, combined additively like `computeExposure` does) can be applied to the collateral posted, and collateral held can now also move as a second, correlated Brownian risk factor (`CollateralAgreement.collateralRiskFactor`), rather than only ever changing via margin calls. Both remain driftless, with no netting set composition changes over time — not a real IMM engine.
