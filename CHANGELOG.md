...
- `CollateralAgreement.marginingStepsPerYear`, decoupling how often collateral is re-margined from the simulation's own `timeStepsPerYear`. Defaults to `null` (margin every simulated step, unchanged behavior); when set, `timeStepsPerYear` must be evenly divisible by it.
- `simulateExposureProfileWithCollateral`, a variant of `simulateExposureProfile` that also simulates collateral re-margining under a new `CollateralAgreement` (symmetric threshold and minimum transfer amount): collateral held starts fully covering the initial exposure and is then a stateful running balance updated by margin calls/returns along the simulated exposure path, with EPE/PFE computed from the post-margining net exposure. `ExposureSimulator.kt`'s path generation and profile aggregation were extracted into small shared internal helpers, reused by both simulation entry points.
- `PiecewiseCreditCurve`, an alternative to `CreditCurve` built from several `CreditCurvePoint`s (tenor, cumulative PD) instead of a single 1-year PD, using a piecewise-constant hazard rate between consecutive points. `CvaAssumptions` gains an optional `cumulativeProbabilityOfDefault` function so `computeMultiPeriodCva` can use it (or any custom curve) instead of the default flat `CreditCurve`, with no change to `computeMultiPeriodCva`'s own parameter list.
